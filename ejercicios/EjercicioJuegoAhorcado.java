...
public class EjercicioJuegoAhorcado {
	...
	public static void main(String[] args) {
		// proceso generar palabra
		String palabra = generarPalabra();

		// repetir el ingreso de letras hasta adivinar o cant de jugadas < 10
	...
	private static String generarPalabra() {
		...
		System.out.println("Ingrese palabra secreta de entre 4 y 10 letras");
		String palabra = sc.next();

		while (palabra.length() < MIN_PALABRA || palabra.length() > MAX_PALABRA) {
			System.out
					.println("La palabra tiene que tener como minimo " + MIN_PALABRA + "y como máximo " + MAX_PALABRA);
	...
	private static int adivinar(String palabra) {
		...
		while (contadorAciertos < palabra.length() && errores != MAX_ERRORES) {
			...
			// si existe, mostrar letra, contar acierto
			palabra.charAt(indiceLetra); // devuelve la letra de la palabra en la posicion indicada
			// sino mostrar _, contar error
			// mostrar _ o _ _ (Errores : 0)
			// volver a pedir letra (bucle)
		}
	...
	private static void mostrarPuntos(int intentosFallidos) {
		...
		// en fc de esos intentos fallidos mostrar puntaje
	...
	/**
	 * Este meodo recibe la palabra a adivinar y devuelve el numero de intentos
